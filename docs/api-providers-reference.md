...
| PersianToolbox (`persian_toolbox_gold24`) | FALLBACK | Free, no key; fair-use quota is not guaranteed | none | Yes | Reference 24K gold in IRR per gram. Unit, time, freshness, and source list are checked before explicit conversion to Toman. No history endpoint. |
...
| PersianToolbox (`persian_toolbox_gold24`) | FALLBACK | رایگان، بدون کلید؛ سهمیه عمومی تضمین نشده | ندارد | بله | طلای مرجع ۲۴ عیار به ریال برای هر گرم. واحد، زمان، تازگی و منبع پیش از تبدیل صریح به تومان سنجیده می‌شوند. تاریخچه ندارد. |
