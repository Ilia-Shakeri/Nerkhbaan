...
| PersianToolbox (`persian_toolbox_btc`) | FALLBACK | Free, no key; fair-use quota is not guaranteed | none | Yes | Iranian wrapper for BTC/USD reference data. Documented five-minute cache accepted without changing the source timestamp; receipt starts only the normal short BTC live window. No history endpoint. |
| PersianToolbox (`persian_toolbox_gold24`) | FALLBACK | Free, no key; fair-use quota is not guaranteed | none | Yes | Reference 24K gold in IRR per gram. Unit, time, freshness, and source list are checked before explicit conversion to Toman. The five-minute instrument window matches the source cache. No history endpoint. |
...
| PersianToolbox | BTC/USD, 24K gold/IRR, and technical USD/IRR reference | Free, no key; fair use and caching required | Integrated; BTC and gold enabled, USD/IRR disabled |
...
| PersianToolbox (`persian_toolbox_btc`) | FALLBACK | رایگان، بدون کلید؛ سهمیه عمومی تضمین نشده | ندارد | بله | پوشش ایرانی برای مرجع BTC/USD. کش مستند پنج‌دقیقه‌ای بدون تغییر زمان اصلی پذیرفته می‌شود؛ زمان دریافت فقط پنجره کوتاه عادی BTC را آغاز می‌کند. تاریخچه ندارد. |
| PersianToolbox (`persian_toolbox_gold24`) | FALLBACK | رایگان، بدون کلید؛ سهمیه عمومی تضمین نشده | ندارد | بله | طلای مرجع ۲۴ عیار به ریال برای هر گرم. واحد، زمان، تازگی و منبع پیش از تبدیل صریح به تومان سنجیده می‌شوند. بازه پنج‌دقیقه‌ای ابزار با کش منبع هماهنگ است. تاریخچه ندارد. |
...
| PersianToolbox | BTC/USD، طلای ۲۴ عیار/ریال و مرجع فنی USD/IRR | رایگان، بدون کلید؛ مصرف منصفانه و cache لازم | یکپارچه؛ BTC و طلا فعال، USD/IRR غیرفعال |
